...
async def test_powo_agent(context, messages):
    ...
    assert messages == [
        ProcessBeginResponse(summary='Analyzing plant data request', data=None),
        ProcessLogResponse(text='Identified: Allium Cepa', data={'genus': 'Allium', 'species': 'Cepa'}),
        ProcessLogResponse(text='Searching Kew Gardens Plants of the World Online(POWO) database.', 
                           data={'genus': 'Allium', 'species': 'Cepa', 'search_url': 'https://powo.science.kew.org/api/2/search?perPage=500&cursor=%2A&q=genus%3AAllium%2Cspecies%3ACepa&f=species_f'}),
        ArtifactResponse(mimetype='text/markdown', 
                         description='Search results for Allium Cepa from POWO', 
        ...
        ProcessLogResponse(text='Retrieving detailed information from Kew Gardens International Plant Names Index(IPNI) for 1 plants.', data=None),
        ProcessLogResponse(text='Successfully retrieved 1 plant details.', data=None),
        ArtifactResponse(mimetype='text/markdown', 
                         description='Detailed botanical information for Allium Cepa from IPNI', 
